...
const parse = () => {
    ...
    if(feature.geometry.type==="LineString"){
      ...
      for (i2 = 0; i2 < arrayCoordinates.length; i2++) {
        ...
        if(arrayCoordinates[i2+1]){
          ...
          let result = compare_line_2_lines(coordinate[0], coordinate[1], arrayCoordinates[i2+1][0], arrayCoordinates[i2+1][1])
          if(result){
            arrayCoordinates.splice(i2+1,0,[result.x, result.y])
            console.log("INSERTED : " +  result);
            console.log("BEFORE : " + obj.features[ii].geometry.coordinates[i2])
            console.log("PUT :" + obj.features[ii].geometry.coordinates[i2+1])
            console.log("AFTER : "+ obj.features[ii].geometry.coordinates[i2+2])
            parse();
          }
        }
      //})
      ...
      for (i1 =0; i1 <arrayCoordinates.length; i1++){
        ...
        for (i3 = 0; i3 < coordinates.length; i3++) {
          ...
          if(coordinates[i3+1]){
            let result = compare_line_2_lines(coordinate[0], coordinate[1], coordinates[i3+1][0], coordinates[i3+1][1])
            if(result){
              coordinates.splice(i3+1,0,[result.x, result.y])
              console.log("INSERTED : " +  result);
              console.log("BEFORE : " + obj.features[ii].geometry.coordinates[i1][i3])
              console.log("PUT :" + obj.features[ii].geometry.coordinates[i1][i3+1])
              console.log("AFTER : "+ obj.features[ii].geometry.coordinates[i1][i3+2])
              parse()
            }
          }
        //})
  ...
  fs.writeFile('./icr-test-with-intersections.json', JSON.stringify(obj, null, 2) , 'utf-8');
};
    ...
    if(feature.geometry.type==="LineString"){
      for (ii2 = 0; ii2 < arrayCoordinates.length; ii2++) {
        ...
        if(arrayCoordinates[ii2+1]){
          let result = checkLineIntersection(lineStartX, lineStartY, lineEndX, lineEndY, coordinate[0], coordinate[1], arrayCoordinates[ii2+1][0], arrayCoordinates[ii2+1][1])
          if(result.onLine1 && result.onLine2){
            ...
            console.log("BEFORE : " + obj.features[iii].geometry.coordinates[ii2])
            console.log("PUT :" + obj.features[iii].geometry.coordinates[ii2+1])
            console.log("AFTER : "+ obj.features[iii].geometry.coordinates[ii2+2])
            return result;
          }
        }
      }
      ...
      for (ii1 =0; ii1 <arrayCoordinates.length; ii1++){
        ...
        for (ii3 = 0; ii3 < coordinates.length; ii3++) {
          ...
          if(coordinates[ii3+1]){
            let result = checkLineIntersection(lineStartX, lineStartY, lineEndX, lineEndY, coordinate[0], coordinate[1], coordinates[ii3+1][0], coordinates[ii3+1][1])
            if(result.onLine1 && result.onLine2){
              console.log("abab : " + result);
              console.log()
              console.log("TEST BEF: "+ lineStartX + ", " + lineStartY)
              ...
              console.log("BEFORE : " + obj.features[iii].geometry.coordinates[ii1][ii3])
              console.log("PUT :" + obj.features[iii].geometry.coordinates[ii1][ii3+1])
              console.log("AFTER : "+ obj.features[iii].geometry.coordinates[ii1][ii3+2])
              return result;
            }
          }
        }
...
function checkLineIntersection(line1StartX, line1StartY, line1EndX, line1EndY, line2StartX, line2StartY, line2EndX, line2EndY) {
    ...
    if(line1EndX === line2StartX && line1EndY === line2StartY)
      return result;
    if(line2EndX ===line1StartX && line2EndY ===line1StartY)
      return result;
